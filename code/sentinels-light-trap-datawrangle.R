# Install biocleanr
pak::pak("iobis/obistools")
pak::pak("HakaiInstitute/biocleanr")
...
master <- merge(
  x = cw_data[, c("stationVisit", "carapace_width")],
  y = dung_count,
  by = "stationVisit",
  all.x = TRUE
) |>
...
# Given that there's a sampling event (the deployment of the LED light trap at several stations) for this project, our schema will consist of an Event Core (Section 1) and two extension data tables:
# occurrence (Section 2), and extended measurementOrFact (Section 3). In the final section (Section 4) we'll do some basic QAQC to ensure the formatted data follows
...
stationVisit <- dung_count |>
  ...
  mutate(
    eventType = "stationVisit",
    eventDate = biocleanr::create_event_date(dung_count$date),
...
individual <- master |>
  ...
  mutate(
    eventID = paste(parentEventID, "m", sep = "-"),
...
kit <- biocleanr::dwc_init(
  "sentinels-light-trap",
  description = "Sentinels of Change Light Trap data",
  standard = "obis"
) |>
  biocleanr::dwc_dataset(
    "Event",
    SOC_event,
    name = "SOC_event"
  )

report <- biocleanr::dwc_validate(kit)
report

# Save locally if validation passes
write_csv(SOC_event, here("obis", "SOC_event_2025.csv"))
...
occ <- left_join(dung_count, stations, by = c("code", "site")) |>
  ...
  pivot_longer(
    metacarcinus_magister_instar:metacarcinus_magister_megalopae,
...
soc_worms <- worrms::wm_records_names(
...
occ <- occ |>
  ...
  group_by(eventID) |>
  mutate(
    occurrenceID = paste(eventID, "occ", row_number(), sep = "-"),
...
ind_cw <- master |>
  ...
  mutate(
    eventID = paste(stationVisit, "m", sep = "-"),
    eventID = paste0(eventID, row_number()),
  ...
  mutate(scientificName = "Metacarcinus magister", lifeStage = "megalopae")

# Add column with scientific name so we can match that to WoRMS. Easy as it's only 1 species!

ind_worms <- worrms::wm_records_names(
  unique(ind_cw$scientificName),
  marine_only = T
) |>
  dplyr::bind_rows() |>
  rename(scientificName = scientificname)
...
# Validate the occurrence table
kit <- kit |>
  biocleanr::dwc_dataset(
    "Occurrence",
    SOC_occ_extension,
    name = "SOC_occ_extension"
  )
...
# Save locally if validation passes
write_csv(SOC_occ_extension, here("obis", "SOC_occ_2025.csv"))
...
emof_project <- project |>
  select(eventID, measurementValue = samplingProtocol) |>
  mutate(
    measurementType = "sampling_method",
...
emof_trap_effort <- stationVisit |>
  select(eventID, measurementValue = sampleSizeValue) |>
  mutate(
    measurementType = "hours_fished",
    measurementID = paste(eventID, measurementType, row_number(), sep = "-"),
    measurementValueID = "",
...
emof_occ <- left_join(SOC_occ, SOC_event, by = "eventID") |>
  ...
  mutate_all(as.character) |>
  pivot_longer(
    cols = individualCount:lifeStage,
  ...
  mutate(
    measurementTypeID = case_when(
      ...
      measurementType ==
        "individualCount" ~ "http://vocab.nerc.ac.uk/collection/P01/current/OCOUNT01/"
    )
  ) |>
  mutate(
    measurementValueID = case_when(
      measurementValue ==
        "instar" ~ "http://vocab.nerc.ac.uk/collection/S11/current/S1105/",
      measurementValue ==
        "megalopae" ~ "http://vocab.nerc.ac.uk/collection/S11/current/S1167/"
    )
  ) |>
  mutate(
  ...
  mutate(
    measurementUnitID = case_when(
      ...
      measurementUnit ==
        "not applicable" ~ "https://vocab.nerc.ac.uk/collection/P06/current/XXXX/"
    )
  )
...
carapace <- merge(
  x = indMeasurement,
  y = SOC_occ_extension[, c("eventID", "occurrenceID")],
  by = "eventID",
  all.x = T
)
...
# Validate the emof table
kit <- kit |>
  biocleanr::dwc_dataset("ExtendedMeasurementOrFact", SOC_emof, name = "emof")

report <- biocleanr::dwc_validate(kit)
report

# Save locally as .csv file:
write_csv(SOC_emof, here("obis", "SOC_emof_2025.csv"))
